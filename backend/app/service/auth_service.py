...
from app.features.blockchain.encryption import normalize_private_key, private_key_hash, private_key_to_public_key
...
class AuthService:
    ...
    def _create_user_account(
        ...
        db_user = models.User(
            ...
            encryption_public_key=private_key_to_public_key(generated_private_key),
    ...
    def migrate_legacy_user_sensitive_fields(cls, user: models.User) -> bool:
        ...
        if not getattr(user, "encryption_public_key", None) and getattr(user, "encrypted_private_key", None):
            try:
                recovered_private_key = cls.decrypt_private_key_from_storage(user.encrypted_private_key)
                user.encryption_public_key = private_key_to_public_key(recovered_private_key)
                changed = True
            except ValueError:
                pass
