...
def decrypt_dek_with_keys(wrapped_dek: str, keys: list[str]) -> bytes:
    for key in keys:
        try:
            return unwrap_dek_with_private_key(wrapped_dek, key)
        except ValueError:
            continue
    raise ValueError("unable to unwrap dek")
...
def _resolve_record_text_payload(record: models.HealthData, private_key: Optional[str] = None) -> str:
    ...
    if record.encrypted_data_content and storage_key:
        try:
            if not record.is_public and record.owner_encrypted_dek and (record.encryption_version or "").lower() == "dek_v1":
                dek = unwrap_dek_with_private_key(record.owner_encrypted_dek, storage_key)
                return decrypt_text_with_dek(record.encrypted_data_content, dek)
...
def _resolve_record_pdf_bytes(record: models.HealthData, private_key: Optional[str] = None) -> bytes:
    ...
    if record.encrypted_pdf_data and storage_key:
        try:
            if not record.is_public and record.owner_encrypted_dek and (record.encryption_version or "").lower() == "dek_v1":
                dek = unwrap_dek_with_private_key(record.owner_encrypted_dek, storage_key)
                return decrypt_binary_with_dek(record.encrypted_pdf_data, dek)
...
def build_private_record_context(record: models.HealthData, keys: list[str]) -> Optional[RecordContext]:
    if record.file_type == "pdf":
        ...
        if not pdf_bytes and record.encrypted_pdf_data:
            if record.owner_encrypted_dek and (record.encryption_version or "").lower() == "dek_v1":
                try:
                    dek = decrypt_dek_with_keys(record.owner_encrypted_dek, keys)
                    pdf_bytes = decrypt_binary_with_dek(record.encrypted_pdf_data, dek)
                except ValueError:
                    pdf_bytes = b""
            else:
                for key in keys:
    ...
    if not payload_text and record.encrypted_data_content:
        if record.owner_encrypted_dek and (record.encryption_version or "").lower() == "dek_v1":
            try:
                dek = decrypt_dek_with_keys(record.owner_encrypted_dek, keys)
                payload_text = decrypt_text_with_dek(record.encrypted_data_content, dek)
            except ValueError:
                return None
        else:
            try:
